...
export async function evaluate(request, ctx) {
  ...
  if (hasUrl) {
    try {
      ...
      return Response.json(
        { ok: false, error: "url must be a public HTTPS URL", attribution: ATTRIBUTION },
        { status: 400 },
      );
    }
  }
  ...
  // Not in the 575. The facilitator catalogs know roughly forty thousand more,
...
function formatServiceResponse(body, status) {
  if (!body.ok || status >= 400) {
    ...
    const detail = body.note || (status === 503
      ...
      : "Provide exactly one valid service slug or public HTTPS URL. No payment should be collected.");
