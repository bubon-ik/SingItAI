/**
 * x402-vet / service - one x402 service, vetted before an agent pays it.
 *
 * The convenience half of x402-vet: same records, same judgement, one lookup by
 * slug or URL. Always runs the live 402 handshake, because a single lookup is
 * exactly the case where freshness is what the caller is buying.
 *
 * Deploy on Bankr x402 Cloud. Self-contained by design - the shared analysis is
 * duplicated from vet-shortlist rather than imported, so each service bundles
 * on its own. Directory facts come from x402-list under CC BY 4.0 and the
...
const SNAPSHOT_PATH = "/x402/vet-service/snapshot.json";
const INDEX_PATH = "/x402/vet-service/index.json";
...
export async function evaluate(request, ctx) {
  ...
  if (
    hasSlug === hasUrl ||
    (hasSlug && (typeof input.slug !== "string" || !input.slug.trim())) ||
    (hasUrl && (typeof input.url !== "string" || !input.url.trim()))
  ) {
    return Response.json(
      { ok: false, error: "exactly one non-empty slug or url is required", attribution: ATTRIBUTION },
      { status: 400 },
    );
  }
  ...
  let urlQuery = null;
  if (hasUrl) {
    // A queried URL is an index key, not a probe target - looking one up sends
    // no request anywhere. So plain HTTP is answerable, and answering is the
    // point: refusing would hide the very finding the caller needs, which is
    // that this endpoint takes payment over a channel anyone can rewrite.
    // Private and local hosts stay refused: they are useless as catalog keys
    // and exist here only as probe bait.
    try {
      urlQuery = publicHttpsUrl(input.url.trim());
    } catch {
      try {
        urlQuery = publicHttpUrl(input.url.trim());
      } catch {
        return Response.json(
          { ok: false, error: "url must be a public http(s) URL", attribution: ATTRIBUTION },
          { status: 400 },
        );
      }
    }
  }
  const slugQuery = hasSlug ? input.slug.trim().toLowerCase() : null;
  ...
  const asOf = new Date(snap.at).toISOString();
  const rated = snap.records.find((r) =>
    slugQuery
      ? String(r.slug ?? "").toLowerCase() === slugQuery
      : r.resource && urlsShareResource(urlQuery, r.resource),
  );

  // A rated service needs no facilitator lookup: the traction directory is the
  // richer source where it has an entry, and skipping the index keeps the
  // common path fast.
  if (rated) {
    const result = applyProbe(rated, await probeService({ slug: rated.slug, url: urlQuery }));
    return Response.json({
      ok: true,
      product: "x402-vet-service",
      asOf,
      source: "x402-list",
      result,
      note: NOTE,
  ...
  // Not in the traction directory. The facilitator catalogs know roughly forty thousand more,
  // and this is exactly the case they exist for.
  const index = await getIndex(ctx, INDEX_PATH, indexBudgetFrom(startedAt));
  const coverage = indexCoverage(index);
  const found = findInIndex(index, urlQuery ?? slugQuery);
    ...
    return Response.json(
      {
        ...
        error: "not_found",
        asOf,
        // Saying how much was searched is the difference between an answer and
        // a misleading one: "unknown" from a partial index is not "absent".
        coverage,
        note: coverage.complete
          ? "Not listed in the traction directory or any indexed facilitator catalog. Sellers register nowhere by design, so this is not evidence the endpoint does not exist."
          : "Not found, but the facilitator index is still being built. Retry once coverage is complete.",
        attribution: ATTRIBUTION,
      },
      { status: 404 },
    );
  ...
  const record = unratedRecord(found.resource, found.entry);
  const result = applyProbe(
    record,
    await probeService({
      url: found.resource,
      method: found.entry.m,
      catalogPayTo: found.entry.t,
    }),
  );
...
function formatServiceResponse(body, status) {
  if (!body.ok || status >= 400) {
    const title = status === 404 ? "Service not found" : status === 503 ? "Service temporarily unavailable" : "Invalid request";
    const detail = body.note || (status === 503
      ? "The service directory could not be refreshed. No payment should be collected."
      : "Provide exactly one valid service slug or public HTTP(S) URL. No payment should be collected.");
    const lines = [`# ${title}`, "", detail];
    if (body.coverage) {
      lines.push("", `Catalog coverage: ${body.coverage.percent ?? "unknown"}% (${body.coverage.endpointsIndexed ?? 0} endpoints indexed).`);
    }
    lines.push("", `Data attribution: ${body.attribution || ATTRIBUTION}`);
    return lines.join("\n");
  }

  const result = body.result;
  const lines = [
    `# ${result.name || "x402 service"}`,
    "",
    `**Verdict: ${verdictLabel(result.verdict)}**`,
    "",
    `Endpoint: ${result.resource || result.probedUrl || "unknown"}`,
    `Price per call: ${priceLabel(result.priceUsd)}`,
    `Networks: ${result.networks?.length ? result.networks.join(", ") : "unknown"}`,
    `Live payment check: ${result.liveProbe || "not checked"}`,
  ];

  if (result.liveProbeLatencyMs !== null && result.liveProbeLatencyMs !== undefined) {
    lines.push(`Response time: ${result.liveProbeLatencyMs} ms`);
  }
  if (result.liveProbeReason) lines.push(`Live check detail: ${result.liveProbeReason}`);
  if (result.livePayTo) lines.push(`Current payment recipient: ${result.livePayTo}`);
  ...
  if (result.liveNetwork) lines.push(`Live network: ${networkName(result.liveNetwork)}`);
  if (result.liveAssetIsUsdc === true) lines.push("Live asset: canonical USDC on Base");
  else if (result.liveAsset) lines.push(`Live asset: ${result.liveAsset}`);
  if (result.liveAmount !== null && result.liveAmount !== undefined) lines.push(`Live atomic amount: ${result.liveAmount}`);

  if (result.why?.length) {
    lines.push("", "Why:", ...result.why.map((reason) => `- ${reason}`));
  }
...
export default async function handler(request, ctx) {
  ...
  return new Response(formatServiceResponse(body, response.status), {
