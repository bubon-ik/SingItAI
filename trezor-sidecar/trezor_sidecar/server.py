...
class _SidecarHttpServer(ThreadingHTTPServer):
    ...
    def pair(self, allow_repair: bool) -> Pairing:
        with self._device_operation_lock:
            return self.service.pair(allow_repair=allow_repair)

    def approve_intent(self, intent: PurchaseIntent, timestamp: int) -> PurchaseIntent:
        with self._device_operation_lock:
            return self.service.approve_intent(intent, timestamp)

    def _run_payment(self, payment_id: str) -> None:
        try:
            with self._device_operation_lock:
                self.service.run_payment(
                    payment_id,
...
class _SidecarHandler(BaseHTTPRequestHandler):
    ...
    def _safe_error(self, error: SafeError) -> None:
        if (
            ...
            or len(error.message) > 160
            or any(ord(character) < 32 for character in error.message)
    ...
    def do_GET(self) -> None:
        path = self._path()
        if path == "/health":
            status = "ready" if self.sidecar_server.settings.enabled else "disabled"
    ...
    def _unsupported_method(self) -> None:
        path = self._path()
        if path == "/health" or path in _POST_PATHS or (
            path is not None
...
def build_server(
    ...
    *,
    ...
    if type(settings.port) is not int or not 0 <= settings.port <= 65_535:
        raise ValueError("sidecar port is invalid")
    if not callable(clock):
        raise ValueError("clock must be callable")
    return _SidecarHttpServer((settings.host, settings.port), settings, service, clock)
